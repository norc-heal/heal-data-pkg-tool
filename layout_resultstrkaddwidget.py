...
class ResultsTrkAddWindow(QtWidgets.QMainWindow):
    ...
    def edit_result(self,checked):
        ...
        versionCheckDf = versionCheckDf[versionCheckDf["trackerType"] == "resultsTracker"]
        ...
        if not versionCheckDf.empty: # at least one json txt annotation file
            ...
            addToMessage = "<br><br>no json txt annotation files available to edit because none exist yet - you must add at least one result to the result tracker before you can edit a result...<br>"
            saveFormat = '<span style="color:red;">{}</span>'
            messageText = addToMessage
            self.userMessageBox.append(saveFormat.format(messageText))
            return 
        ...
        if self.w is None:
            ...
            self.w = ScrollAnnotateResultWindow(workingDataPkgDirDisplay=self.workingDataPkgDirDisplay, workingDataPkgDir=self.workingDataPkgDir, filesCheckList=filesCheckList, mode="edit")
            self.w.show()
            self.w.load_file()
    ...
    def annotate_result_based_on(self,checked):
        ...
        if not versionCheckDf.empty: # at least one json txt annotation file
            ...
            print("no json txt annotation files")
            addToMessage = "<br><br>no json txt annotation files available to be used as the basis for a new annotation file because none exist yet - you must add at least one result to the result tracker before you can edit a result...<br>"
            saveFormat = '<span style="color:red;">{}</span>'
            messageText = addToMessage
            self.userMessageBox.append(saveFormat.format(messageText))
            return 
        ...
        if self.w is None:
            #self.w.editState = True
            self.w = ScrollAnnotateResultWindow(workingDataPkgDirDisplay=self.workingDataPkgDirDisplay, workingDataPkgDir=self.workingDataPkgDir, filesCheckList=filesCheckList, mode="add-based-on")
            self.w.show()
            self.w.load_file()
